...
@Service
public class AzomeWriter
{
 
    @Autowired
    DataSource ds;

    public int insert(Azome... entities) {
        try {
            // 准备SQL语句
            String sql = "INSERT INTO t_zone (file_id,\ntitle,\ncontent,\nimage,\ngeometry,\nfrom_time,\nend_time,\ncreate_time,\nupdate_time,\ncreate_by,\nupdate_by\n) VALUES (\n?,\n?,\n?,\n?,\n?,\n?,\n?,\n?,\n?,\n?,\n?\n)";
    ...
    public int update(Azome... entities) {
        try {
            // 准备SQL语句
            String sql = "UPDATE t_zone SET title=?,\ncontent=?,\nimage=?,\ngeometry=?,\nfrom_time=?,\nend_time=?,\ncreate_time=?,\nupdate_time=?,\ncreate_by=?,\nupdate_by=?\n WHERE file_id=? \n;";
            QueryRunner qr = new QueryRunner(this.ds);
            int result = 0;
            for (var entity : entities) {
                // 实例化QueryRunner并执行DML语句
    ...
    public int delete(int... ids) {
        try {
            // 准备SQL语句
            String sql = "DELETE FROM t_zone WHERE file_id=? \n;";
            QueryRunner qr = new QueryRunner(this.ds);
            int result = 0;
            for (var id : ids) {
                // 实例化QueryRunner并执行DML语句
